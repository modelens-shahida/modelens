...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    async function initAuth() {
      ...
      if (storedToken) {
        ...
        try {
          if (storedToken === "demo-session-token-placeholder") {
            setUser(storedUser ? JSON.parse(storedUser) : {
              id: 1,
              email: "indrabathini86@gmail.com",
              full_name: "indra",
              role: "admin",
              credits: 1000
            });
  ...
  const login = async (email, password, redirectTo = "/dashboard") => {
    ...
    try {
      ...
      try {
        ...
        console.warn("Backend offline or auth failed, falling back to demo session:", backendError);
        const demoToken = "demo-session-token-placeholder";
        localStorage.setItem("modelens_token", demoToken);
        setToken(demoToken);
        ...
        profile = {
          id: 1,
          email: email || "indrabathini86@gmail.com",
          full_name: email ? email.split("@")[0] : "indra",
          role: "admin",
          credits: 1000
        };
      }
  ...
  const register = async (email, password, full_name, redirectTo = "/dashboard") => {
    ...
    try {
      ...
      try {
        ...
        console.warn("Backend offline or registration failed, falling back to demo session:", backendError);
        const demoToken = "demo-session-token-placeholder";
        localStorage.setItem("modelens_token", demoToken);
        setToken(demoToken);
  ...
  const refreshUser = async () => {
    try {
      if (token === "demo-session-token-placeholder") {
        return user;
      }
      const profile = await api.get("/api/v1/auth/me");
      setUser(profile);
