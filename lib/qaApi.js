...
export const qaApi = {
  ...
  /**
   * Get defect heatmap and hotspot matrix for a QA evaluation.
   * @param {number} evaluationId
   */
  async getHeatmap(evaluationId) {
    return api.get(`/api/v1/qa/evaluations/${evaluationId}/heatmap`);
  },

  /**
   * Set configurable QA thresholds for a brand.
   * @param {Object} payload
   * @param {number} payload.brand_id
   * @param {Object} payload.thresholds
   */
  async setBrandThresholds(payload) {
    return api.post("/api/v1/qa/brand-thresholds", payload);
  },

  /**
   * Get default brand QA hard gate thresholds.
   */
  async getDefaultThresholds() {
    return api.get("/api/v1/qa/brand-thresholds/defaults");
  },
};
