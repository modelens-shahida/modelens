    ...
    <div className="min-h-screen bg-black text-white p-6">
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="mb-6">
          ...
          <p className="text-zinc-400 text-sm mt-1">Manage your profile, preferences, and workspace settings</p>
        </div>
        ...
        {/* Brand Selector */}
        {brands.length > 0 && (
          <div className="flex items-center gap-3 bg-zinc-900/40 border border-zinc-800 rounded-2xl p-6">
            <label className="text-sm font-semibold text-white">Selected Workspace Brand:</label>
            <select value={selectedBrandId} onChange={(e) => setSelectedBrandId(e.target.value)} className="bg-zinc-950 border border-zinc-800 rounded-xl px-3 py-2 text-sm text-zinc-200 outline-none focus:border-purple-500">
            ...
            <div className="flex gap-2 mb-4">
              <div className="flex-1 flex items-center gap-2 bg-zinc-800 border border-zinc-700 rounded-xl px-3 py-2">
                ...
                <input
                  ...
                  className="flex-1 bg-transparent text-sm text-zinc-200 outline-none placeholder-zinc-650"
                />
              </div>
              <button
                onClick={handleAddDomain}
                disabled={addingDomain || !domainInput.trim()}
                className="flex items-center gap-1.5 bg-purple-600 hover:bg-purple-700 disabled:opacity-50 px-4 py-2 rounded-xl text-sm font-medium transition text-white"
              >
            ...
            {loadingDomains ? (
              ...
              <div className="space-y-2">
                {ssoDomains.map(domain => (
                  <div key={domain.id || domain} className="flex items-center justify-between bg-zinc-850/50 border border-zinc-700 rounded-xl px-4 py-2.5">
